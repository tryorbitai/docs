...
let agent = Agent(
    role: "Data Analyst",
    purpose: "Analyze business data and generate insights", 
    context: "Expert in statistical analysis and business intelligence",
    tools: ["calculator", "fileRead", "webSearch"],
    memory: true
)
```

#### 3. Knowledge & Memory Integration
Agents can access shared knowledge bases and maintain persistent memory:
```swift
let knowledgeBase = KnowledgeBase()
try await knowledgeBase.addSource(DirectorySource(path: "/documents"))

agent.knowledgeBase = knowledgeBase
agent.enableMemory(type: .longTerm)
```

#### 4. Task Definition & Assignment
...
let task = Task(
    ...
    expectedOutput: "Executive summary with key insights and recommendations",
    tools: ["fileRead", "calculator", "dataVisualization"],
    guardrails: [.noSensitiveData, .factualAccuracy]
)
```
...
let orbit = try Orbit(
    ...
    tasks: [analysisTask, researchTask, reportTask],
    process: .hierarchical, 
    manager: seniorAnalyst,
    memory: true,
    usageMetrics: true
)
```
