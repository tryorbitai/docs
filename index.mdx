...
```mermaid
graph TD
    A[User Request] --> B[Orbit Orchestrator]
    B --> C[Agents - Workers]
    B --> D[Process Management]
    C --> E[Tools - External Capabilities]
    D --> F[Tasks - Work Units]
    E --> F
    F --> G[LLM Provider + Memory + KnowledgeBase + Guardrails]
    G --> H[Validated Results]
```
